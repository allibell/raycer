...
namespace {
  ...
  public:
    inline RenderThread(std::shared_ptr<Raycer> rt, Buffer<unsigned int>& b)
      : raycer(rt),
        buffer(b)
    {
    }
...
RenderWidget::RenderWidget(QWidget* parent, std::shared_ptr<Raycer> raycer)
  : QWidget(parent),
    m_raycer(raycer),
...
void RenderWidget::render() {
  stop();
  m_raycer->uncancel();
  p->renderThread = new RenderThread(m_raycer, *p->buffer);
...
void RenderWidget::markRectsInProgress(QImage& image) const {
  for (const auto& rect : p->renderThread->raycer->activeRects()) {
